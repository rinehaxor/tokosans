...
import { sendAdminNotification, getAdminWaNumber } from '../../../../lib/wa-client';

export const POST: APIRoute = async () => {
  const adminNumber = getAdminWaNumber();
  if (!adminNumber) {
    return new Response(
      JSON.stringify({ ok: false, message: 'ADMIN_WA_NUMBER belum dikonfigurasi di server.' }),
  ...
  return new Response(
    JSON.stringify(
      sent
        ? { ok: true, message: 'Pesan tes berhasil dikirim ke nomor admin.' }
