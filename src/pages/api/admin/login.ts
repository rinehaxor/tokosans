...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  if (!email || !password) return redirect('/admin/login?error=1');
  try {
    const { data, error } = await getSupabaseAuth().auth.signInWithPassword({ email, password });
    if (error || !data.session) return redirect('/admin/login?error=1');
  ...
  } catch (error) { console.error('Supabase admin login error:', error); return redirect('/admin/login?error=1'); }
};